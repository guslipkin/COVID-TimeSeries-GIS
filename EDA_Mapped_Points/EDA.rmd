...
for(i in 1:length(countyList)) {
  countyList[[i]] <- data.table(date = seq(from = as.Date("2020-01-022"), to = as.Date("2021-06-07"), by = 'day'), totalCases = countyList[[i]])
  ...
  countyList[[i]] <- countyList[[i]] %>%
    group_by(month, year) %>%
    summarise(meanNewCases = mean(newCases)) %>%
    filter(year == 2020)
}
...
for(i in 1:length(countyList)) {
  countyList[[i]] <- data.table(meanNewCases = countyList[[i]])[, c("temp", "soil", "precip") := .(monthTemp[[i]], monthSoil[[i]], monthPrecip[[i]])]
}
