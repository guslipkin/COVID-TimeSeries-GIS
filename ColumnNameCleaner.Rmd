...
newCols <- c(cols[!grepl("[0-9]{2,}", cols)], paste0(as.character(str_replace_all(mdy(cols[grepl("[0-9]{2,}", cols)]), "-", "_"))))
# # assign the new column names
# colnames(df) <- newCols
colnames(df) <- newCols
# write the dataframe back to the original document
#write.csv(df, "../COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv")
```
